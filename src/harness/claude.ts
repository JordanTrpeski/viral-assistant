...
export class ClaudeHarness implements DevelopmentHarness {
  ...
  async launch(request: HarnessLaunchRequest): Promise<HarnessRunResult> {
    const command = commandParts(this.command, ["-p", "--output-format", "stream-json", "--verbose", "--permission-mode", "bypassPermissions", "--max-turns", "60", ...(request.model ? ["--model", request.model] : [])]);
